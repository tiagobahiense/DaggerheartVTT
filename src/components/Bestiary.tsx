  ...
  const handleSaveToBank = async (token: Token) => {
      ...
      const updatedEnemies = sessaoData.saved_enemies.map((enemy: any) => {
          ...
          if (enemy.name === token.name && enemy.img === token.img) {
              return { 
                  ...enemy, 
                  // Salva os stats, mas garante que PV/PF venham cheios no template
                  stats: { ...currentStats, currentPV: currentStats.maxPV, currentPF: currentStats.maxPF } 
              };
          }
          return enemy;
      });
      ...
      alert("Ficha salva no Banco com sucesso! Novos tokens virão preenchidos.");
  };
